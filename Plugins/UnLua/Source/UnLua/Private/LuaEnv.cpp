...
namespace UnLua
{
    ...
    bool FLuaEnv::LoadBuffer(const char* Buffer, const size_t Size, const char* InName , const char* fileName)
    {
        // TODO: env support
...
#endif
            return LoadBuffer(Buffer + 3, Size - 3, InName , fileName);
        }
#endif
    ...
    int FLuaEnv::LoadFromFileSystem(lua_State* L)
    {
        ...
        auto LoadIt = [&]
        {
            if (Env.LoadString(Data, TCHAR_TO_UTF8(*FullPath), FileName))
                return 1;
