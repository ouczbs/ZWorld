	...
	, RendererDynamicSettings(MakeVoxelShared<FVoxelRendererDynamicSettings>())
{
	...
	{
		if (!bAutomaticallyRefreshFoliage || !SpawnerConfig)
		{
			return;
		}

		if (SpawnerConfig == Object || SpawnerConfig->NeedsToRebuild(Object, PropertyChangedEvent))
		{
...
void AVoxelWorld::SetOffset(const FIntVector& OffsetInVoxels)
{
	...
	*WorldOffset = OffsetInVoxels;
	...
	InstancedMeshManager->RecomputeMeshPositions();
}
...
UVoxelMultiplayerInterface* AVoxelWorld::CreateMultiplayerInterfaceInstance()
{
	...
	if (IsCreated())
	{
		FVoxelMessages::Error(FUNCTION_ERROR("Can't call once the Voxel World is created!"), this);
		return nullptr;
	}
	
	if (!bEnableMultiplayer)
	{
		FVoxelMessages::Error(FUNCTION_ERROR("bEnableMultiplayer = false!"), this);
		return nullptr;
	}
	
	auto* const Class = MultiplayerInterface.Get();
	if (!Class)
	{
		FVoxelMessages::Error(FUNCTION_ERROR("Invalid MultiplayerInterface class!"), this);
		return nullptr;
	}
	if (Class->HasAnyClassFlags(CLASS_Abstract))
	{
		FVoxelMessages::Error(FUNCTION_ERROR("MultiplayerInterface class is abstract!"), this);
		return nullptr;
	}
	if (MultiplayerInterfaceInstance)
	{
		FVoxelMessages::Error(FUNCTION_ERROR("MultiplayerInterfaceInstance already created!"), this);
		return MultiplayerInterfaceInstance;
	}

	MultiplayerInterfaceInstance = NewObject<UVoxelMultiplayerInterface>(this, Class, NAME_None, RF_Transient);
	return MultiplayerInterfaceInstance;
}
...
TVoxelSharedRef<FVoxelMultiplayerManager> AVoxelWorld::CreateMultiplayerManager() const
{
	VOXEL_FUNCTION_COUNTER();
	return FVoxelMultiplayerManager::Create(FVoxelMultiplayerSettings(
		this,
		Data.ToSharedRef(),
		DebugManager.ToSharedRef(),
		LODManager.ToSharedRef()));
}

TVoxelSharedRef<FVoxelInstancedMeshManager> AVoxelWorld::CreateInstancedMeshManager() const
{
	VOXEL_FUNCTION_COUNTER();
	return FVoxelInstancedMeshManager::Create(FVoxelInstancedMeshManagerSettings(this, Pool.ToSharedRef(), EventManager.ToSharedRef()));
}

TVoxelSharedRef<IVoxelSpawnerManager> AVoxelWorld::CreateSpawnerManager() const
{
	VOXEL_FUNCTION_COUNTER();
	return FVoxelSpawnerManager::Create(FVoxelSpawnerSettings(
		this,
		PlayType,
		Pool.ToSharedRef(),
		DebugManager.ToSharedRef(),
		...
		LODManager.ToSharedRef(),
		Renderer.ToSharedRef(),
		InstancedMeshManager.ToSharedRef(),
		EventManager.ToSharedRef()));
}
...
void AVoxelWorld::CreateWorldInternal(const FVoxelWorldCreateInfo& Info)
{
	...
	*WorldOffset = FIntVector::ZeroValue;
	...
	InstancedMeshManager = CreateInstancedMeshManager();
	SpawnerManager = CreateSpawnerManager();
		
	if (bEnableMultiplayer)
	{
		if (PlayType == EVoxelPlayType::Game)
		{
...
void AVoxelWorld::DestroyWorldInternal()
{
	...
	InstancedMeshManager->Destroy();
	FVoxelUtilities::DeleteTickable(GetWorld(), InstancedMeshManager);

	SpawnerManager->Destroy();
	FVoxelUtilities::DeleteTickable(GetWorld(), SpawnerManager);

	if (MultiplayerManager.IsValid())
	{
		MultiplayerManager->Destroy();
		FVoxelUtilities::DeleteTickable(GetWorld(), MultiplayerManager);
	}
